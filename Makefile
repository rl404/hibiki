...
.PHONY: swagger
swagger:
	@$(SWAG_GEN) --parseVendor -g cmd/hibiki/main.go -o ./docs
...
# Build and run cron update manga.
.PHONY: cron-update
cron-update: build
	@cd $(CMD_PATH); \
	./$(BINARY_NAME) cron update

# Build and run cron fill missing manga.
.PHONY: cron-fill
cron-fill: build
	@cd $(CMD_PATH); \
...
docker-build:
	...
	@$(DOCKER_IMAGE) prune -f --filter label=stage=hibiki_builder

# Start built docker containers for api.
.PHONY: docker-api
docker-api:
	@$(COMPOSE_CMD) -f $(COMPOSE_API) -p hibiki-api up -d
	@$(COMPOSE_CMD) -f $(COMPOSE_API) -p hibiki-api logs --follow --tail 20

# Start built docker containers for consumer.
.PHONY: docker-consumer
docker-consumer:
	@$(COMPOSE_CMD) -f $(COMPOSE_CONSUMER) -p hibiki-consumer up -d
	@$(COMPOSE_CMD) -f $(COMPOSE_CONSUMER) -p hibiki-consumer logs --follow --tail 20

# Start built docker containers for cron update manga.
.PHONY: docker-cron-update
docker-cron-update:
	@$(COMPOSE_CMD) -f $(COMPOSE_CRON_UPDATE) -p hibiki-cron-update up

# Start built docker containers for cron fill missing manga.
.PHONY: docker-cron-fill
docker-cron-fill:
	@$(COMPOSE_CMD) -f $(COMPOSE_CRON_FILL) -p hibiki-cron-fill up

# Start built docker containers for migrate.
.PHONY: docker-migrate
docker-migrate:
	@$(COMPOSE_CMD) -f $(COMPOSE_MIGRATE) -p hibiki-migrate up

# Start docker to run lint check.
.PHONY: docker-lint
docker-lint:
	@$(COMPOSE_CMD) -f $(COMPOSE_LINT) -p hibiki-lint run --rm hibiki-lint $(GCL_RUN) -D errcheck --timeout 5m
...
docker-update:
	@$(COMPOSE_CMD) -f $(COMPOSE_API) -p hibiki-api up -d
	@$(COMPOSE_CMD) -f $(COMPOSE_CONSUMER) -p hibiki-consumer up -d
	@$(DOCKER_IMAGE) prune -f --filter label=stage=hibiki_binary

# Stop running docker containers.
.PHONY: docker-stop
docker-stop:
	@$(COMPOSE_CMD) -f $(COMPOSE_API) -p hibiki-api stop
	@$(COMPOSE_CMD) -f $(COMPOSE_CONSUMER) -p hibiki-consumer stop